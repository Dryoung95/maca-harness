...
def make_kernel_wmma7(BM: int, BN: int, BK: int, WM: int, WN: int):
    nwarps_m = BM // (16 * WM)
    nwarps_n = BN // (16 * WN)
...
// Only store_matrix_sync needs a hand-written replacement: it is a no-op in
// MACA 3.3.0. Both loads use the library, but only its scalar (row_major)
// variants — the col_major variants use vectorized double reads that scramble
// data on this toolchain.
...
__global__ void mm_wmma7_kernel(const __half* __restrict__ A,
                                const __half* __restrict__ B,
                                float* __restrict__ C, int n) {{
    // sA is transposed (K major) so the col_major a-load reads it correctly
    ...
    for (int kt = 0; kt < numKTiles; kt++) {{
        // sA is [BK][BM+8] (transposed). Element (m, k) of the tile goes to
        // sA[k][m]; the col_major a-load then reads x[i] = sA[col+i, row].
        #pragma unroll
        ...
        for (int kk = 0; kk < BK; kk += 16) {{
            wmma::fragment<wmma::matrix_a, 16, 16, 16, __half, wmma::col_major> a[WM];
            #pragma unroll
            for (int i = 0; i < WM; i++)
                wmma::load_matrix_sync(a[i],
                    &sA[kk][warpM * (16 * WM) + i * 16], BM + 8);
...
torch::Tensor mm_wmma7_{BM}_{BN}_{BK}_{WM}_{WN}(torch::Tensor A, torch::Tensor B) {{
...
def wmma7_name(BM, BN, BK, WM, WN):
    return f"mm_wmma7_{BM}_{BN}_{BK}_{WM}_{WN}"
